...
// Clients
func (n *nutsify) Login() (string, error) {
	panic("not implemented") // TODO: Implement
}

func (n *nutsify) GetFuelLevel(clientID string) (uint, error) {
...
func (n *nutsify) SetMode(clientID string, automaticRefill bool) error {
	// 0. Get subscription for response channel.
	var commandService commandService
	subscription := n.redisClient.Subscribe(commandService.BuildCommandResponseRedisChannel(clientID))
	defer subscription.Close()
...
func (n *nutsify) StartRefill(clientID string) error {
	// 0. Get subscription for response channel.
	var commandService commandService
	subscription := n.redisClient.Subscribe(commandService.BuildCommandResponseRedisChannel(clientID))
	defer subscription.Close()
...
func (n *nutsify) Stop(clientID string) error {
	// 0. Get subscription for response channel.
	var commandService commandService
	subscription := n.redisClient.Subscribe(commandService.BuildCommandResponseRedisChannel(clientID))
	defer subscription.Close()
...
func (n *nutsify) ForceShutdown(clientID string) error {
	// 0. Get subscription for response channel.
	var commandService commandService
	subscription := n.redisClient.Subscribe(commandService.BuildCommandResponseRedisChannel(clientID))
	defer subscription.Close()
