...
func (n *nutsify) FetchNextCommand() string {
...
func (n *nutsify) CompleteCommand(commandResponse CommandResponse) error {
...
func validateCompleteCommandParameters(commandResponse CommandResponse) bool {
	if commandResponse.RequestID == "" {
		log.Printf("nutsify.CompleteCommand(): empty requestID found [%+v]", commandResponse)
		return false
	}

	if commandResponse.Error == "" && commandResponse.Result == nil {
		log.Printf("nutsify.CompleteCommand(): both result and error fields are empty [%+v]", commandResponse)
		return false
	}
